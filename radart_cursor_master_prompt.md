...
RadArt is not a simple news summarizer.
...
### Storage
- SQLite file: `backend/data/radart.db`
...
The scraper must:
- use a custom user agent: `RadArtBot/1.0`;
...
DATABASE_PATH=backend/data/radart.db
```
...
### Explanation
`RadArt detects rising media signals, filters them through each client brief, and turns trends into actionable campaign recommendations.`
